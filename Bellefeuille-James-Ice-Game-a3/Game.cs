...
namespace Game10003
{
    public class Game
    {
        Vector2 mouseBrush = new Vector2(50, 10); //size of the brush
        Vector2 mousePosition = Vector2.Zero; //self explanitory
        Color colBrown = new Color(180, 100, 90); //custom colors
        Color colUiBack = new Color(50, 20, 100);
        Color colIceBlue = new Color(179, 240, 245);
        Ice ice = new Ice(); //ice
        Debris[] obstacles = new Debris[20]; //debris
        BackGroundTile[] backGround = new BackGroundTile[150]; //background
        ...
        double distance = 0; //current distance traveled
        string distanceUnits = ""; // distance unit (mm, cm, m, km);
        int money = 0;
        int upgradeIceSize = 1; // The upgrades
        ...
        public bool hitBoxDetect(Vector2 hitBoxPosition, Vector2 hitBoxSize, Vector2 hitboxPosition2, Vector2 hitboxSize2) // function for hitboxes.
        { // detects if a hitbox is breached
            bool LeftOf;
            bool rightOf;
        ...
        { // calculates the distances to accurate measurements
            if (distance < 26.4583)
        ...
        public void Update()
        {
            ...
            { // code for the title screen
                ...
                Text.Draw("Super Cool Ice Game 100% Swag!", 210, 100);
                Text.Draw("Professional Main Menu", 260, 400);
                Text.Size = 20;
                Text.Draw("By James Bellefeuille", 290, 150);
            ...
            else
            {
                if (startGame)
                {  // the game will start
                    foreach (BackGroundTile tile in backGround) // background tileing and movement
                    {
                        tile.DrawTile();
                        tile.OffScreen();
                        tile.Move(-backgroundSpeed);
                    }
                    double distance2 = distanceCalculator(distance); // gets distance to display ingame
                    ...
                    Draw.Ellipse(mousePosition, mouseBrush); // this will draw a brush on the mouse
                    Draw.LineSize = 1;
                    foreach (Debris deb in obstacles)  // this checks the debris collisions
                    { // thank you forEach!!!
                        if (deb.DebrisCollide(mousePosition, mouseBrush.X / 3)) //checks mouse collision
                        { 
                        ...
                        {
                            ice.size -= (float)0.012 * deb.avgSize; // slows and shrinks the ice
